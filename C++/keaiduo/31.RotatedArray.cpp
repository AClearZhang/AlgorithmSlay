/*
 * @FilePath            : \project\AlgorithmSlay\C++\keaiduo\31.RotatedArray.cpp
 * @Author              : AClearZhang
 * @Date                : 2021-12-06 23:12:20
 * @E-mail              : aclearzhang@qq.com
 * @Homepage            : www.aclear.top
 * @LastEditors         : AClearZhang
 * @LastEditTime        : 2021-12-07 00:02:44
 * @Version             : 1.0
 * @Description         : 
 */
/*
...
public:
    ...
    int searchTar(vector<int>& nums, int target) {
        if(nums[0] == target){
            return 0;
        }
        if( nums.size() == 1 ){
        ...
        while( l <= r ){
            ...
            //     return mid;
            // }
            if( mid < nums.size()-1  && nums[mid]>nums[0] && nums[mid+1] > nums[0]){
                l =  mid + 1;
            }else if(mid < nums.size()-1  && nums[mid] < nums[0] && nums[mid+1] < nums[0]){
                r = mid;
            }
            else if(mid < nums.size()-1  && nums[mid] >= nums[0] && nums[mid+1] <= nums[0]){
                break;
...
int main(int argc, char const *argv[])
{
    ...
    int target = 5;
    Solution a;
    cout << a.searchTar(nums, target) << endl;
    system("pause");
    return 0;
}
